...
def scrape_masoko(query):
    ...
    if response.status_code != 200:
        print(f"Failed to fetch data from Masoko. Status code: {response.status_code}")
        return

    soup = BeautifulSoup(response.text, 'html.parser')

    retailer, _ = Retailer.objects.get_or_create(name="Masoko", url="https://www.masoko.com")

    product_containers = soup.select('li.product-item')
